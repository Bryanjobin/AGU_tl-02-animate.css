...
RGraph.Activity = function(conf) {
    var id = conf.id, canvas = document.getElementById(id), min = conf.min, max = conf.max, value = conf.value;
    // id, min, max, value
    ...
    this.draw = function() {
        ...
        this.radius = Math.min((this.canvas.width - this.marginLeft - this.marginRight) / 2, (this.canvas.height - this.marginTop - this.marginBottom) / 2);
        //
        ...
        if (typeof properties.width === "string") this.width += Number(properties.width);
        //
    ...
    this.drawBackground = function() {
        ...
        // Draw the grid?
        if (properties.backgroundGrid) // Draw the background grey circles
        {
            if (properties.backgroundGridCircles) {
                // How many background circles?
                if (typeof properties.backgroundGridCirclesCount === "number") var count = properties.backgroundGridCirclesCount;
                else var count = this.value.length + 1;
                for(var i = 0; i < count; i++){
                    var radius = this.radius - i * (this.width + 2 * properties.marginInner);
                    this.path("b a % % % 0 6.29 false s %", this.centerx, this.centery, radius, properties.backgroundGridColor);
                }
                //
                ...
                if (properties.backgroundGridRadials) {
                    ...
                    for(var i = 0; i <= properties.backgroundGridRadialsCount; ++i)// Radius must be greater than 0 for Opera to work
                    this.path("b    a % % % % % %     a % % % % % %    s %", this.centerx, this.centery, radius, i * angle - RGraph.HALFPI, i * angle - RGraph.HALFPI, false, this.centerx, this.centery, this.radius, i * angle - RGraph.HALFPI, i * angle - RGraph.HALFPI, false, properties.backgroundGridColor);
                }
            }
        }
    };
    ...
    this.drawMeter = function() {
        ...
        if (properties.backgroundRings) {
            ...
            for(var i = 0; i < this.value.length; ++i){
                // Determine the color
                if (RGraph.isArray(properties.backgroundRingsColors) && typeof properties.backgroundRingsColors[i] === "string") color = properties.backgroundRingsColors[i];
                else color = properties.colors[i];
        ...
        for(var i = 0; i < this.value.length; ++i){
            ...
            this.path("b       a % % % % % false          a % % % % % true       f %", this.centerx, this.centery, r - i * (properties.marginInner + properties.marginInner + this.width), 0 - RGraph.HALFPI, radians - RGraph.HALFPI, this.centerx, this.centery, r - this.width - i * (properties.marginInner + properties.marginInner + this.width), radians - RGraph.HALFPI, 0 - RGraph.HALFPI, properties.colors[i]);
            // Store the coordinates of the ring
            this.angles[i] = this.coords[i] = {
                x: this.centerx,
                y: this.centery,
    ...
    this.drawLabels = function() {
        ...
        if (typeof properties.labels === "string") properties.labels = RGraph.arrayPad({
            array: [],
        ...
        for(var i = 0; i < properties.labels.length; ++i){
            if (typeof properties.labels === "object" && properties.labels.length && typeof properties.labels[i] === "string") var text = RGraph.labelSubstitution({
                object: this,
            ...
            RGraph.text({
                object: this,
                text: RGraph.isString(properties.labels[i]) || RGraph.isNumber(properties.labels[i]) ? text : "",
    ...
    this.drawIcons = function() {
        for(var i = 0; i < this.value.length; ++i)if (typeof properties.icons[i] === "string" && properties.icons[i].length) {
            ...
            this.images[i].onload = function() {
                ...
                obj.context.drawImage(this, x, y, width, height);
            };
        }
    };
    //
    ...
    this.drawEnds = function() {
        ...
        for(var i = 0; i < this.coords.length; ++i){
            ...
            this.path("b a % % % % % false f %", x, y, this.width / 2, 0, 6.29, properties.colors[i]);
            // cx,cy,angle,radius
            ...
            this.path("b a % % % % % false f %", xy[0], xy[1], (this.coords[i].radiusOuter - this.coords[i].radiusInner) / 2, 0, 6.29, properties.colors[i]);
        }
    };
    //
    ...
    this.getShape = function(e) {
        ...
        for(var i = 0; i < this.coords.length; ++i){
            ...
            this.path("b    a % % % % % false    a % % % % % true", this.coords[i].x, this.coords[i].y, this.coords[i].radiusOuter, this.coords[i].angleStart, this.coords[i].angleEnd, this.coords[i].x, this.coords[i].y, this.coords[i].radiusInner, this.coords[i].angleEnd, this.coords[i].angleStart);
            //
            // Add the ends to the path if the bars are capped
            //
            if (properties.ends) {
                this.path("a % % % % % false", this.coords[i].x, this.coords[i].y - this.coords[i].radiusInner - this.width / 2, this.width / 2, RGraph.HALFPI, RGraph.PI + RGraph.HALFPI);
                ...
                this.path("m % %    a % % % % % false", xy[0], xy[1], xy[0], xy[1], (this.coords[i].radiusOuter - this.coords[i].radiusInner) / 2, 0, 6.29);
            }
    ...
    this.getObjectByXY = function(e) {
        var mouseXY = RGraph.getMouseXY(e), width = this.width;
        // Work out the radius
        var radius = RGraph.getHypLength(this.centerx, this.centery, mouseXY[0], mouseXY[1]);
        if (radius > this.radius || radius < this.radius * 0.25) return null;
        return this;
    };
    //
    ...
    this.adjusting_mousemove = function(e) {
        ...
        if (properties.adjustable && RGraph.Registry.get("adjusting") && RGraph.Registry.get("adjusting").uid == this.uid) {
            ...
            } else index = RGraph.Registry.get("adjusting.index");
    ...
    this.getIndexByRadius = function(radius) {
        ...
        for(var i = 0; i < this.coords.length; ++i){
            if (radius < this.coords[i].radiusOuter && radius > this.coords[i].radiusInner) return i;
        }
        return null;
    };
    //
    ...
    this.pathBar = function(index) {
        // Path the outer edge
        this.path("a % % % % % false", this.coords[index].x, this.coords[index].y, this.coords[index].radiusOuter, 0 - RGraph.HALFPI, this.coords[index].angleEnd);
        ...
        if (properties.ends === "round") this.path("a % % % % % false", xy[0], xy[1], this.width / 2, this.coords[index].angleEnd, this.coords[index].angleEnd + RGraph.PI);
        // Path the inner edge
        this.path("a % % % % % true", this.coords[index].x, this.coords[index].y, this.coords[index].radiusInner, this.coords[index].angleEnd, this.coords[index].angleStart);
        // Path the starting curved end if the ends are round
        if (properties.ends === "round") this.path("a % % % % % false", this.centerx, this.centery - this.coords[index].radiusInner - (this.coords[index].radiusOuter - this.coords[index].radiusInner) / 2, this.width / 2, RGraph.HALFPI, RGraph.PI + RGraph.HALFPI);
    };
    //
    ...
    this.positionTooltipStatic = function(args) {
        var obj = args.object, e = args.event, tooltip = args.tooltip, index = args.index, canvasXY = RGraph.getCanvasXY(obj.canvas), coords = this.coords[args.index];
        //cx,cy,angle,radius
        ...
        args.tooltip.style.left = canvasXY[0] + point[0] - tooltip.offsetWidth / 2 + obj.properties.tooltipsOffsetx + "px";
        args.tooltip.style.top = canvasXY[1] + point[1] - tooltip.offsetHeight - 10 + obj.properties.tooltipsOffsety + "px";
        // If the top of the tooltip is off the top of the page
        // then move the tooltip down
        if (parseFloat(args.tooltip.style.top) < 0) args.tooltip.style.top = "5px";
    };
    //
    ...
    this.parseColors = function() {
        ...
        if (colors && colors.length) for(var i = 0; i < colors.length; ++i)colors[i] = this.parseSingleColorForGradient(colors[i]);
    };
    //
    ...
    this.parseSingleColorForGradient = function(color) {
        ...
        if (color.match(/^gradient\((.*)\)$/i)) {
            ...
            // Create the gradient
            var grad = this.context.createLinearGradient(properties.marginLeft, 0, this.canvas.width - properties.marginLeft - properties.marginRight, 0);
            var diff = 1 / (parts.length - 1);
            grad.addColorStop(0, RGraph.trim(parts[0]));
            for(var j = 1, len = parts.length; j < len; ++j)grad.addColorStop(j * diff, RGraph.trim(parts[j]));
        }
    ...
    this.grow = function() {
        ...
        if (RGraph.isNull(this.currentValue)) {
            this.currentValue = [];
            for(var i = 0; i < this.value.length; ++i)this.currentValue[i] = this.min;
        }
        var opt = arguments[0] || {}, frames = opt.frames || 30, frame = 0, diff = [], step = [], callback = arguments[1] || function() {}, initial = [];
        ...
        function iterator() {
            ...
            if (frame <= frames) RGraph.Effects.updateCanvas(iterator);
            else callback(obj);
        }
        iterator();
        return this;
...
//# sourceMappingURL=index.14c1d432.js.map
