...
(function(win, doc, undefined) {
    ...
    RGraph.drawTitle = function() {
        var args = RGraph.getArgs(arguments, "object"), obj = args.object, halign = "center", valign = "center", x = (obj.canvas.width - obj.properties.marginLeft - obj.properties.marginRight) / 2 + obj.properties.marginLeft, y = null, textConf = RGraph.getTextConf({
            object: args.object,
            prefix: "title"
        });
        ...
        //
        if (!RGraph.isNumber(obj.properties.titleSize)) textConf.size += 4;
        if (!RGraph.isBoolean(obj.properties.titleBold)) textConf.bold = true;
        // Determine the Y coordinate
        y = obj.properties.marginTop - textConf.size - 5;
        if (obj.properties.xaxisPosition === "top") y = obj.canvas.height - obj.properties.marginBottom + textConf.size + 5;
        //
        // Vertically center the text if the key is not present
        // or if it's not positioned in the margin
        //
        if (obj.properties.key && obj.properties.key.length && obj.properties.keyPosition && obj.properties.keyPosition !== "margin") var valign = "center";
        ...
        if (RGraph.isString(obj.properties.titleSubtitle) && obj.properties.titleSubtitle.length > 0) valign = "bottom";
        else valign = "center";
        //
        ...
        if (typeof obj.properties.titleSubtitle === "string" && obj.properties.titleSubtitle.length) {
            ...
            if (RGraph.isNull(obj.properties.titleSubtitleSize)) obj.properties.titleSubtitleSize = textConf.size - 4;
            ...
            if (properties.backgroundGrid) {
                ...
                if (properties.backgroundGridHlines) {
                    ...
                    for(y = marginTop; y <= height; y += hsize){
                        ...
                        if (args.object.properties.variant === "3d" && args.object.type === "bar" && args.object.properties.backgroundGridThreedYaxis) {
                            cacheContext.moveTo(marginLeft, Math.round(y));
                            cacheContext.lineTo(marginLeft - args.object.properties.variantThreedOffsetx, Math.round(y) + args.object.properties.variantThreedOffsety);
                        }
                    }
                }
    ...
    RGraph.draw3DAxes = function() {
        ...
        // X axis
        if (xaxis) {
            if (xaxispos === "center") args.object.path("b m % % l % % l % % l % % c s #aaa f %", marginLeft, marginTop + halfGraphArea, marginLeft + offsetx, marginTop + halfGraphArea - offsety, args.object.canvas.width - marginRight + offsetx, marginTop + halfGraphArea - offsety, args.object.canvas.width - marginRight, marginTop + halfGraphArea, properties.variantThreedXaxisColor);
            else {
                ...
                args.object.path("m % % l % % l % % l % % c s #aaa f %", marginLeft, xaxisYCoord, marginLeft + offsetx, xaxisYCoord - offsety, args.object.canvas.width - marginRight + offsetx, xaxisYCoord - offsety, args.object.canvas.width - marginRight, xaxisYCoord, properties.variantThreedXaxisColor);
            }
        }
    };
    //
    ...
    RGraph.draw3DYAxis = function(args) {
        var args = RGraph.getArgs(arguments, "object");
        var properties = args.object.properties;
        var marginLeft = args.object.marginLeft, marginRight = args.object.marginRight, marginTop = args.object.marginTop, marginBottom = args.object.marginBottom, xaxispos = properties.xaxisPosition, graphArea = args.object.canvas.height - marginTop - marginBottom, halfGraphArea = graphArea / 2, offsetx = properties.variantThreedOffsetx, offsety = properties.variantThreedOffsety, yaxisFill = properties.variantThreedYaxisColor || "#ddd";
        // Y axis
        ...
        args.object.path("b m % % l % % l % % l % % s #aaa f % b", x, marginTop, x + offsetx, marginTop - offsety, x + offsetx, args.object.canvas.height - marginBottom - offsety, x, args.object.canvas.height - marginBottom, properties.variantThreedYaxisColor);
    //}
    };
    //
    ...
    RGraph.createUID = function() {
        return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function(c) {
            var r = Math.random() * 16 | 0, v1 = c == "x" ? r : r & 0x3 | 0x8;
    ...
    RGraph.text = function(args) {
        ...
        function domtext() {
            ...
            if (!obj.canvas.rgraph_domtext_wrapper) {
                ...
                if ((obj.type === "bar" || obj.type === "bipolar" || obj.type === "hbar") && obj.properties.variant === "3d") wrapper.style.transform = "skewY(" + obj.properties.variantThreedAngle + "rad)";
    ...
    RGraph.path = function(args) {
        ...
        for(var i = 0, len = p.length; i < len; i += 1)switch(p[i]){
            case "b":
                context.beginPath();
                break;
            case "c":
            ...
            case "s":
                ...
                break;
            case "f":
                if (p[i + 1]) context.fillStyle = p[i + 1];
                context.fill();
                i++;
                break;
            case "qc":
                context.quadraticCurveTo(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]));
                i += 4;
                break;
            case "bc":
                context.bezierCurveTo(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]), parseFloat(p[i + 5]), parseFloat(p[i + 6]));
                i += 6;
                break;
            case "r":
                context.rect(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]));
                i += 4;
                break;
            case "a":
                context.arc(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]), parseFloat(p[i + 5]), p[i + 6] === "true" || p[i + 6] === true || p[i + 6] === 1 || p[i + 6] === "1" ? true : false);
                i += 6;
                break;
            case "at":
                context.arcTo(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]), parseFloat(p[i + 5]));
                i += 5;
                break;
            case "lw":
                ...
                break;
            case "e":
                context.ellipse(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]), parseFloat(p[i + 5]), parseFloat(p[i + 6]), parseFloat(p[i + 7]), p[i + 8] === "true" ? true : false);
                i += 8;
                break;
            case "lj":
            ...
            case "ss":
                ...
                break;
            case "fr":
                context.fillRect(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]));
                i += 4;
                break;
            case "sr":
                context.strokeRect(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]));
                i += 4;
                break;
            case "cl":
                context.clip();
            ...
            case "ro":
                ...
                break;
            case "tf":
                context.transform(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]), parseFloat(p[i + 5]), parseFloat(p[i + 6]));
                i += 6;
                break;
            case "stf":
                context.setTransform(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]), parseFloat(p[i + 5]), parseFloat(p[i + 6]));
                i += 6;
                break;
            case "cr":
                context.clearRect(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]));
                i += 4;
                break;
            case "ld":
    ...
    RGraph.clipTo.start = function() {
        ...
        if (typeof args.dimensions === "string") {
            if (args.dimensions === "tophalf") args.object.path("sa b r % % % % cl", 0, 0, args.object.canvas.width, args.object.canvas.height / 2);
            ...
            else if (args.dimensions === "tophalf.margins") args.object.path("sa b r % % % % cl", 0, 0, args.object.canvas.width, (args.object.canvas.height - args.object.properties.marginTop - args.object.properties.marginBottom) / 2 + args.object.properties.marginTop);
                ...
                args.object.path("sa b r % % % % cl", 0, (args.object.canvas.height - args.object.properties.marginTop - args.object.properties.marginBottom) / 2 + args.object.properties.marginTop, args.object.canvas.width, args.object.canvas.height);
            //
            ...
            // the chart
            //
            } else if (args.dimensions === "lefthalf.margins") args.object.path("sa b r % % % % cl", 0, 0, args.object.properties.marginLeft + (args.object.canvas.width - args.object.properties.marginLeft - args.object.properties.marginRight) / 2, args.object.canvas.height);
            else if (args.dimensions === "righthalf.margins") args.object.path("sa b r % % % % cl", args.object.properties.marginLeft + (args.object.canvas.width - args.object.properties.marginLeft - args.object.properties.marginRight) / 2, 0, (args.object.canvas.width - args.object.properties.marginLeft - args.object.properties.marginRight) / 2, args.object.canvas.height);
    ...
    RGraph.parseConfigString = function() {
        ...
        // Default to a comma
        args.separator ??= ",";
        var insideQuote = false, entries = [], entry = [], parts = {};
        args.string.split("").forEach(function(character) {
            if (character === '"' || character === "'") insideQuote = !insideQuote;
            else if (character === args.separator && !insideQuote) {
                entries.push(entry.join(""));
                entry = [];
            } else entry.push(character);
        });
        entries.push(entry.join(""));
        for(let i1 = 0; i1 < entries.length; ++i1){
            var index = entries[i1].indexOf("=");
            var name = entries[i1].substr(0, index);
            var value = entries[i1].substr(index + 1);
    ...
    RGraph.drawHorizontalLines = function(obj) {
        var lines = obj.properties.horizontalLines, avg, x, y, label, halign, valign, hmargin = 10, vmargin = 5, position, textFont, textSize, textColor, textBold, textItalic, data, linewidth;
        if (lines) {
            //
            ...
            for(let i1 = 0; i1 < obj.properties.horizontalLines.length; ++i1){
                var conf = lines[i1], textFont = conf.labelFont || obj.properties.textFont, textColor = conf.labelColor || defaults.labelColor, textSize = conf.labelSize || obj.properties.textSize - 4, textBold = typeof conf.labelBold === "boolean" ? conf.labelBold : obj.properties.textBold, textItalic = typeof conf.labelItalic === "boolean" ? conf.labelItalic : obj.properties.textItalic;
                switch(obj.type){
                    case "line":
                        // Calculate the Y coord if we've been
                        // given a numeric value
                        if (typeof conf.value === "number") y = obj.getYCoord(conf.value);
                    ...
                    case "bar":
                        ...
                        if (typeof conf.value === "number") y = obj.getYCoord(conf.value);
                            ...
                            var total = 0;
                            obj.data.map((v1)=>{
                                if (RGraph.isNumber(v1)) total += v1;
                                else if (RGraph.isArray(v1)) total += RGraph.arraySum(v1);
                            });
                            var num = 0;
                            for(let i1 = 0; i1 < obj.data.length; ++i1){
                                if (obj.properties.grouping === "grouped") num += RGraph.isArray(obj.data[i1]) ? obj.data[i1].length : 1;
                                else if (obj.properties.grouping === "stacked") ++num;
                            }
                            avg = total / num;
                            y = obj.getYCoord(avg);
                        }
                    ...
                    case "scatter":
                        ...
                        if (typeof conf.value === "number") y = obj.getYCoord(conf.value);
                            ...
                            var sum = 0;
                            obj.data[0].map((v1)=>sum += parseFloat(v1[1]));
                ...
                //
                linedash = "[1,1]";
                if (conf.dotted === true) linedash = "[1,3]";
                if (conf.dashed === true || typeof conf.dashed === "undefined" && typeof conf.dotted === "undefined") linedash = "[6,6]";
                //
                // Draw the line
                //
                obj.path("lw % ld % b m % % l % % s %", typeof conf.linewidth === "number" ? conf.linewidth : defaults.linewidth, linedash, obj.properties.marginLeft, y, obj.canvas.width - obj.properties.marginRight, y, conf.color || defaults.color);
                //
                // Draw the label
                //
                ...
                if ([
                    ...
                    if (!conf.labelPosition) conf.labelPosition = defaults.labelPosition;
    ...
    RGraph.isNumeric = function(value) {
        value = String(value);
        return Boolean(value.match(/^[-.0-9]+$/)) || Boolean(value.match(/^[-.0-9]+e[-0-9]+$/)) || value === "Infinity" || value === "-Infinity" || Boolean(value.match(/^[-.0-9]+x[0-9a-f]+$/i));
    };
...
//
if (typeof "".substr !== "function") String.prototype.substr = function(start, length) {
...
//# sourceMappingURL=index.c073d8c2.js.map
